...
export default function EditProjectClient({ project }: { project: Project }) {
    ...
    const [form, setForm] = useState({
        ...
        target_profit_margin: project.target_profit_margin.toString(),
        status: project.status,
        category: project.category || 'مشاريع الحج',
    ...
    return (
        <div className="p-8 max-w-3xl mx-auto" dir="rtl">
            <div className="mb-6">
                ...
                <h1 className="text-2xl font-bold text-gray-900">تعديل بيانات المشروع</h1>
                <p className="text-gray-500 text-sm mt-1">قم بتحديث معلومات مشروع &quot;{project.name}&quot;</p>
            </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <Input className={inputClass} value={form.client} onChange={e => set('client', e.target.value)} required />
                            </div>
                            <div className="space-y-2">
                                <Label className={labelClass}>قيمة العقد (ريال) *</Label>
                                <Input className={inputClass} type="number" value={form.total_value} onChange={e => set('total_value', e.target.value)} required />
                            </div>
                            <div className="space-y-2">
                                <Label className={labelClass}>نسبة الربح المستهدفة (%)</Label>
                                <Input className={inputClass} type="number" value={form.target_profit_margin} onChange={e => set('target_profit_margin', e.target.value)} />
                            </div>
                            <div className="space-y-2">
